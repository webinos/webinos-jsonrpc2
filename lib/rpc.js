...
(function () {
	...
	var handleResponse = function (response) {
		...
		if (typeof this.awaitingResponse[response.id] !== 'undefined'){
            ...
            // The JSON-RPC response is not waiting response
            if (this.callbackObjects[response.id] !== 'undefined') {
                // This is a pending operation.
                if (typeof response.error !== 'undefined' && response.error != null) {
                    // This is an error response.
                    if (response.error.data) {
                        if (response.error.data.name === 'SecurityError') {
                            if (this.callbackObjects[response.id].onSecurityError) {
                                this.callbackObjects[response.id].onSecurityError(response.error.data);
                            }
                        } else {
                            logger.log('Received error response with error name.' +
                                    response.error.data.name);
                        }
                    } else {
                        logger.log('Received error response with no error data.');
                    }
                }
            }
        }
