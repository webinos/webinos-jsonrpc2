	...
	_RPCHandler.prototype.handleMessage = function (jsonRPC, from, msgid){
		...
		function doRPC() {
			if (typeof jsonRPC.method !== 'undefined' && jsonRPC.method != null) {
				...
				handleRequest.call(self, jsonRPC, from, msgid);
			} else {
				// received message is RPC response
				handleResponse.call(self, jsonRPC, from, msgid);
			}
		}
		...
		if (this.checkPolicy) {
			...
			this.checkPolicy(jsonRPC, from, msgid, function(isAllowed) {
				...
				if (!isAllowed) {
					...
					self.executeRPC(rpc, undefined, undefined, from, msgid);
				} else {
