...
(function () {
	...
	var handleRequest = function (request, from) {
		var service, serviceId, method;

		// regex matches: foo@bar.baz foo@bar.baz.baz foo@.baz bar.baz
		var matched = request.method.match(/(?:(.+)@)?([^\.]*)\.(.*)/);
		if (matched[1]) {
			service = matched[1];
			serviceId = matched[2];
		} else {
		...
		if (!service || !method) {
			...
			logger.log("err: cannot handle request, wrong method format");
			return;
		}
		...
		var includingObject;
		if (serviceId || /^internal:\/\//.test(service)) {
			includingObject = this.registry.getServiceWithTypeAndId(service, serviceId);
		} else {
			// check if it's a request to object registered with registerCallbackObject
	...
	_RPCHandler.prototype.createRPC = function (service, method, params) {
		...
		if (service.api && Object.prototype.toString.call(service.id) == '[object String]') {
