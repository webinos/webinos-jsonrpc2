...
(function () {
	 ...
	 * @param jsonRPC The RPC message coming in.
	 * @param from The sender.
	 * @param msgid An id.
	 */
	_RPCHandler.prototype.handleMessage = function (jsonRPC, from, msgid) {
		...
		// Add a permission attribute to the JSON RPC notation.
		if (typeof module !== 'undefined') {
			jsonRPC.params = jsonRPC.params || [];
			jsonRPC.params[3] = true;  // permission element.
		}
		...
		if (this.checkPolicy) {
			...
			if (!isAllowed) {
				// permission is not a regular parameter and used to pass the
				// permission to Service Discovery.
				if (typeof module !== 'undefined') {
					jsonRPC.params[3] = false; // permission element.
				}
				
				if (jsonRPC.method === 'ServiceDiscovery.findServices') {
					handleRequest.call(this, jsonRPC, from, msgid);
					return;
				}
			
				var rpc = newJSONRPCResponseError(
						jsonRPC.id,
						{
							name:	 'SecurityError',
							code:	 18,
