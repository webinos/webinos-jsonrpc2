...
(function () {
	...
	Registry.prototype.getServiceWithTypeAndId = function(serviceTyp, serviceId) {
		...
		if (!receiverObjs) receiverObjs = [];

		if (receiverObjs.length > 0 && !serviceId) {
			// handles internal api services like ServiceDiscovery
			return receiverObjs[0];
		}

		for (var i=0; i<receiverObjs.length; i++) {
			if (receiverObjs[i].id === serviceId) {
				return receiverObjs[i];
			};
		}

		// no service for this type/id
		return undefined;
	};
